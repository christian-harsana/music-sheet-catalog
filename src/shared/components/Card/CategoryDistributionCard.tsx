...
export default function CategoryDistributionCard({ title, data, isLoading } : CategoryDistributionCardProps) {
                            ...
                            <div className="mb-4">
                                <PieChart
                                    ...
                                    >
                                        {
                                            data.map((data, index) => (
                                                <Cell key={`cell-${data.id}`} className={`${FILL_COLOR_CLASSES[index % FILL_COLOR_CLASSES.length]}`} />
                                            ))
                                        }
                                    </Pie>
                                    <Tooltip 
                                ...
                                <ul className="flex flex-wrap gap-x-5 gap-y-2 justify-center">
                                    {
                                        data.map((d, index) => 
                                            <li key={!d.id ? `0` : d.id} className="flex gap-2">
                                                ...
                                                <div className="text-left font-semibold">{!d.name ? `No genre` : d.name} ({d.count})</div>
                                            </li>
                                        )
                                    }
                                </ul>
