...
function ModalOverlay({ children }: { children: ReactNode }) {
	return (
		<div
			className="fixed top-0 left-0 z-30 flex justify-center items-center w-screen h-screen bg-black/75"
			aria-live="polite"
		>
			{children}
		</div>
...
export function UIProvider({ children }: { children: ReactNode }) {
	...
	return (
		<UIContext.Provider value={{ addToast, removeToast, showModal, closeModal }}>
			...
			{notificationList && (
				<ToastContainer notificationList={notificationList} handleRemoveToast={removeToast} />
			)}
		</UIContext.Provider>
	);
}
