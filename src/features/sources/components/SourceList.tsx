	...
	const showEditForm = (source: Source) => {
		showModal(
			<Modal title={'Edit Source'}>
				<SourceForm source={source} refreshData={refreshSources} />
			</Modal>,
		);
	};
	...
	const showDeleteConfirmation = (id: string, name: string) => {
		showModal(
			<Modal title={'Confirmation'}>
				<DeleteConfirmation id={id} name={name} refreshData={refreshSources} />
			</Modal>,
		);
	};
						...
						{isLoading ? (
							...
							sources.map((source) => (
								...
								>
									...
									<td
										role="cell"
										className="block px-3 pb-1 font-semibold before:w-15 before:inline-block before:content-[attr(data-title)':'] before:me-1.5 before:font-normal md:table-cell md:pt-2 md:pb-2 md:before:content-none md:font-normal"
										data-title="Author"
									>
										{source.author}
									</td>
