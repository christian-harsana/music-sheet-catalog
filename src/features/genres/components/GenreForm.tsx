...
export default function GenreForm({ genre, refreshData }: GenreFormProps) {
	...
	function validateForm(GenreFormData: GenreFormData): GenreFormDataError {
		...
		Object.keys(GenreFormData).forEach((name) => {
			...
			const error = validateField(fieldName, GenreFormData[fieldName]);
	...
	const handleGenreFormSubmit = async (
		e: React.FormEvent,
		...
		if (isErrorExist) {
			...
			const result = !genreId
				? await createGenre(genreFormData, token)
				: await updateGenre(genreId, genreFormData, token);
