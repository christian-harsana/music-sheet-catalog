	...
	const showEditForm = (level: Level) => {
		showModal(
			<Modal title={'Edit Level'}>
				<LevelForm level={level} refreshData={refreshLevels} />
			</Modal>,
		);
	};
	...
	const showDeleteConfirmation = (id: string, name: string) => {
		showModal(
			<Modal title={'Confirmation'}>
				<DeleteConfirmation id={id} name={name} refreshData={refreshLevels} />
			</Modal>,
		);
	};
								...
								<td role="cell" className="block px-3 pt-2 pb-4 md:table-cell md:pt-2 md:pb-2">
									<div className="flex flex-nowrap gap-3">
										...
										<button
											type="button"
											className="px-2 py-1 border border-violet-500 hover:border-violet-600 rounded-md bg-violet-500 hover:bg-violet-600 text-sm text-gray-50"
											onClick={() => showDeleteConfirmation(level.id, level.name)}
										>
											Delete
										</button>
									</div>
