	...
	useEffect(() => {
		const fetchSheets = async () => {
			...
			try {
				...
				if (filters.key && filters.key.trim() !== 'all')
					filterParams.append('keyQuery', filters.key);
				if (filters.level && filters.level.trim() !== 'all')
					filterParams.append('levelQuery', filters.level);
				if (filters.genre && filters.genre.trim() !== 'all')
					filterParams.append('genreQuery', filters.genre);
